...
class LoginPage extends RunnerPage
{
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("tbluser")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$strSQL = "select * from ".$this->connection->addTableWrappers( "tbluser" )
			." where "
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "tbluser", $data, $blobfields, $this );
			if( !$retval )
				return false;
