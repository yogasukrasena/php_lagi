...
class display
{
	...
	function displayTotalsReport($tableprefix,$total_type,$tableheaders,$date1,$date2,$where1,$where2)
	{
		...
		$users_table="$tableprefix".'tbluser';
	...
	function formatData($field,$data,$tableprefix)
	{
		...
		{
			$field_first_name=$this->idToField("$tableprefix".'tbluser','first_name',$data);
			$field_last_name=$this->idToField("$tableprefix".'tbluser','last_name',$data);
			return $field_first_name.' '.$field_last_name;
		}
