...
$query = "SELECT * FROM tblEstruder";
$FieldBox = $_POST['Estruder'];
...
function openDialog(url, width, height, returnFunc, args) {
	if (!dialogWin.win || (dialogWin.win && dialogWin.win.closed)) {
		...
		dialogWin.returnedValue_EstruderCode = ""
		dialogWin.args = args
		dialogWin.url = url
...
function setPrefs_ColorCodi() {
	...
	document.c_codificationSearch.Estruder.value = dialogWin.returnedValue_Estruder
}
</script>
</head>
		...
		<td>
			...
			<table cellpadding="3" cellspacing="0" class="InLineFormTABLE">
				...
				<!-- BEGIN Row -->
  				...
  				echo"<tr>
    				<td class=\"InLineDataTD\"><input type=\"hidden\" name=\"EstruderCode\" value=\"$alldata[EstruderCode]\" />
					<a href=\"#\" onClick=\"javascript:window.opener.document.SampCeramicForm.Estruder.value=document.EstruderPopup.EstruderCode.value;window.close()\" class=\"InLineDataLink\" >$alldata[EstruderCode]	</a></td>
