...
//$tabel = $_POST['tabel'];
	...
	$TextureNotes    = $_POST['TextureNotes'];
		...
		$Update = "UPDATE tblTexture SET tblTexture.TextureTechDraw = '$TechDraw' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_FILES['Photo1']['name'] == null){
		...
		$Update = "UPDATE tblTexture SET tblTexture.TexturePhoto1 = '$Photo1' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_FILES['Photo2']['name'] == null){
		...
		$Update = "UPDATE tblTexture SET tblTexture.TexturePhoto2 = '$Photo2' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_FILES['Photo3']['name'] == null){
		...
		$Update = "UPDATE tblTexture SET tblTexture.TexturePhoto3 = '$Photo3' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_FILES['Photo4']['name'] == null){
		...
		$Update = "UPDATE tblTexture SET tblTexture.TexturePhoto4 = '$Photo4' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	...
	If (!$_POST['DelTechDraw']== null){
		...
		$Update = "UPDATE tblTexture SET tblTexture.TextureTechDraw = '' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_POST['DelPhoto1']== null){
		$nama = $_POST["DelPhoto1"];
		unlink($UploadDir."/$nama");
		$Update = "UPDATE tblTexture SET tblTexture.TexturePhoto1 = '' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_POST['DelPhoto2']== null){
		$nama = $_POST["DelPhoto2"];
		unlink($UploadDir."/$nama");
		$Update = "UPDATE tblTexture SET tblTexture.TexturePhoto2 = '' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_POST['DelPhoto3']== null){
		$nama = $_POST["DelPhoto3"];
		unlink($UploadDir."/$nama");
		$Update = "UPDATE tblTexture SET tblTexture.TexturePhoto3 = '' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_POST['DelPhoto4']== null){
		$nama = $_POST["DelPhoto4"];
		unlink($UploadDir."/$nama");
		$Update = "UPDATE tblTexture SET tblTexture.TexturePhoto4 = '' where tblTexture.ID = '$id';";
		mysql_query($Update);
	}
	
$_rs ="UPDATE tblTexture 
			SET 
			tblTexture.TextureDate = '$TextureDate',  
	tblTexture.TextureNotes    = '$TextureNotes'
	 WHERE tblTexture.ID = '$id';";
	
	$query=mysql_query($_rs);
	
	if ($query)
	{
		echo "<p>record has been updated click </p> <a href=\"view_rndTexture.php\"> here </a> to continue";
	}
	else
	{
		$sala=mysql_error();
