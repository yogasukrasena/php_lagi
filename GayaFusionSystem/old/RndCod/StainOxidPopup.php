...
$query = "SELECT * FROM tblStainOxid";
$FieldBox = $_POST['StainOxid'];
...
function openDialog(url, width, height, returnFunc, args) {
	if (!dialogWin.win || (dialogWin.win && dialogWin.win.closed)) {
		...
		dialogWin.returnedValue_StainOxidCode = ""
		dialogWin.args = args
		dialogWin.url = url
...
function setPrefs_ColorCodi() {
	...
	document.c_codificationSearch.StainOxid.value = dialogWin.returnedValue_StainOxid
}
</script>
</head>
...
<table width="765" border="0" cellspacing="0" cellpadding="3">
	<tr>
		<td>
		    <font class="InLineFormHeaderFont">List of StainOxid </font><br>
			Total Records : <?php echo"<strong>$jumlah</strong>";?> &nbsp;<br>
		</td>
	</tr>
	<tr>
		<td>
			<form name="StainOxidPopup">
			<table cellpadding="3" cellspacing="0" class="InLineFormTABLE">
				...
				<!-- BEGIN Row -->
  				...
  				echo"<tr>
    				<td class=\"InLineDataTD\"><input type=\"hidden\" name=\"StainOxidCode\" value=\"$alldata[StainOxidCode]\" />
					<a href=\"#\" onClick=\"javascript:window.opener.document.SampCeramicForm.StainOxid.value=document.StainOxidPopup.StainOxidCode.value;window.close()\" class=\"InLineDataLink\" >$alldata[StainOxidCode]	</a></td>
