...
$query = "SELECT * FROM tblTools";
$FieldBox = $_POST['Tools'];
...
function openDialog(url, width, height, returnFunc, args) {
	if (!dialogWin.win || (dialogWin.win && dialogWin.win.closed)) {
		...
		dialogWin.returnedValue_ToolsCode = ""
		dialogWin.args = args
		dialogWin.url = url
...
function setPrefs_ColorCodi() {
	...
	document.c_codificationSearch.Tools.value = dialogWin.returnedValue_Tools
}
</script>
</head>
		...
		<td>
			...
			<table cellpadding="3" cellspacing="0" class="InLineFormTABLE">
				...
				<!-- BEGIN Row -->
  				...
  				echo"<tr>
    				<td class=\"InLineDataTD\"><input type=\"hidden\" name=\"ToolsCode\" value=\"$alldata[ToolsCode]\" />
					<a href=\"#\" onClick=\"javascript:window.opener.document.SampCeramicForm.Tools.value=document.ToolsPopup.ToolsCode.value;window.close()\" class=\"InLineDataLink\" >$alldata[ToolsCode]	</a></td>
