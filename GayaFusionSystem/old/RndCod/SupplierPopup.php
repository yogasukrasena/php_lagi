...
function openDialog(url, width, height, returnFunc, args) {
	if (!dialogWin.win || (dialogWin.win && dialogWin.win.closed)) {
		...
		dialogWin.returnedValue_SupplierCode = ""
		dialogWin.args = args
		dialogWin.url = url
...
function setPrefs_ColorCodi() {
	...
	document.c_codificationSearch.Supplier.value = dialogWin.returnedValue_Supplier
}
</script>
</head>
			...
			<table cellpadding="3" cellspacing="0" class="InLineFormTABLE">
				...
				<!-- BEGIN Row -->
  				<?php
					...
					$tu = $_GET['ID'];
  				echo"<tr>
    				<td class=\"InLineDataTD\"><input type=\"text\" name=\"SupplierCode\" value=\"$alldata[SupCode]\" />
					<a href=\"#\" onClick=\"javascript:window.opener.document.DesMaterialForm.DmSupplier.value=document.SupplierPopup.SupplierCode[$tu].value;window.close()\" class=\"InLineDataLink\" >$alldata[SupCode]	</a></td>
