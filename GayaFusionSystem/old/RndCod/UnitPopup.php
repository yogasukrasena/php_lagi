...
function openDialog(url, width, height, returnFunc, args) {
	if (!dialogWin.win || (dialogWin.win && dialogWin.win.closed)) {
		...
		dialogWin.returnedValue_UnitID = ""
		dialogWin.args = args
		dialogWin.url = url
...
function setPrefs_ColorCodi() {
	...
	document.c_codificationSearch.Unit.value = dialogWin.returnedValue_Unit
}
</script>
</head>
	...
	<tr>
		<td>
			...
			<table cellpadding="3" cellspacing="0" class="InLineFormTABLE">
				<tr>
    				<td width="71" nowrap class="InLineColumnTD"><a class="InLineSorterLink" href="#">UNIT ID</a></td>
    				<td width="250" nowrap class="InLineColumnTD"><a class="InLineSorterLink" href="#">UNIT NAME</a>&nbsp;</td>
  				</tr>
				<!-- BEGIN Row -->
  				<?php
  					while ($alldata = mysql_fetch_array($sqlcari)){ //ini nampilin data
  				echo"<tr>
    				<td class=\"InLineDataTD\"><input type=\"hidden\" name=\"UnitID\" value=\"$alldata[UnitID]\" />
					<a href=\"#\" onClick=\"javascript:window.opener.document.DesMaterialForm.DmUnit.value=document.UnitPopup.UnitID.value;window.close()\" class=\"InLineDataLink\" >$alldata[UnitID]	</a></td>
    				<td class=\"InLineDataTD\" >$alldata[UnitValue]</td> 
  				</tr>";
				}
				echo "<tr>";
  					echo"<td class=\"InLineFooterTD\" colspan=\"2\">";
