...
//$tabel = $_POST['tabel'];
	...
	If (!$_FILES['TechDraw']['name'] == null){
		...
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobeTechDraw = '$TechDraw' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_FILES['Photo1']['name'] == null){
		...
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobePhoto1 = '$Photo1' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_FILES['Photo2']['name'] == null){
		...
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobePhoto2 = '$Photo2' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_FILES['Photo3']['name'] == null){
		...
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobePhoto3 = '$Photo3' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_FILES['Photo4']['name'] == null){
		...
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobePhoto4 = '$Photo4' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	...
	If (!$_POST['DelTechDraw']== null){
		...
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobeTechDraw = '' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_POST['DelPhoto1']== null){
		$nama = $_POST["DelPhoto1"];
		unlink($UploadDir."/$nama");
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobePhoto1 = '' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_POST['DelPhoto2']== null){
		$nama = $_POST["DelPhoto2"];
		unlink($UploadDir."/$nama");
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobePhoto2 = '' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_POST['DelPhoto3']== null){
		$nama = $_POST["DelPhoto3"];
		unlink($UploadDir."/$nama");
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobePhoto3 = '' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	If (!$_POST['DelPhoto4']== null){
		$nama = $_POST["DelPhoto4"];
		unlink($UploadDir."/$nama");
		$Update = "UPDATE tblEngobe SET tblEngobe.EngobePhoto4 = '' where tblEngobe.ID = '$id';";
		mysql_query($Update);
	}
	
$_rs ="UPDATE tblEngobe 
			SET 
			tblEngobe.EngobeDate = '$EngobeDate',  
	tblEngobe.EngobeNotes    = '$EngobeNotes'
	 WHERE tblEngobe.ID = '$id';";
	
	$query=mysql_query($_rs);
	
	if ($query)
	{
		echo "<p>record has been updated click </p> <a href=\"view_rndEngobe.php\"> here </a> to continue";
	}
	else
	{
		$sala=mysql_error();
