...
If (!empty($add)){
	If ((!empty($CostPerMinute)) || (!empty($PiecesPerHour))){
		$dataedit = "UPDATE tblCosting_RakuBisque SET 
			tblCosting_RakuBisque.CostPerMinute = '$CostPerMinute',
			tblCosting_RakuBisque.PiecesPerHour = '$PiecesPerHour';";
		$edit = mysql_query($dataedit);	
	}
}
else{
	$AddData = "INSERT INTO tblCosting_RakuBisque (CostPerMinute, PiecesPerHour) VALUE ('$CostPerMinute', '$PiecesPerHour')";
