...
If (!empty($add)){
	If ((!empty($CostPerMinute)) || (!empty($PiecesPerHour))){
		$dataedit = "UPDATE tblCosting_Finishing SET 
			...
			tblCosting_Finishing.PiecesPerHour = '$PiecesPerHour';";
		$edit = mysql_query($dataedit);	
	}
}
else{
	$AddData = "INSERT INTO tblCosting_Finishing (CostPerMinute, PiecesPerHour) VALUE ('$CostPerMinute', '$PiecesPerHour')";
