...
class clsMenu {
  ...
  function TransformToFlat($MenuArray, & $Result, $Level = 1) {
    foreach ($MenuArray as $MenuNode) {
      ...
      $MenuChildren = isset($MenuNode["CCS_Children"]) ? $MenuNode["CCS_Children"] : null;
  ...
  function GetTreeNodes($Component) {
    ...
    while($this->DataSource->next_record()) {
      ...
      if (isset($CurrentNode[$this->ParentField]) && !$this->CompareFields($CurrentNode[$this->ParentField], $this->RootId)) {
  ...
  function HasChildren($item_id, $array) {
    foreach ($array as $part) {
      if (isset($part[$this->ParentField]) && ($part[$this->ParentField] == $item_id)) {
        return true;
      }
    }
    return false;
...
class clsControl
{
  ...
  function GetLink()
  {
    ...
    if($this->Parameters == "") {
      ...
      $Delimeter = strpos($this->Page, "?") === false ? "?" : "&";
      if ($CCSUseAmp) {
...
//clsFileUpload Class @0-E95823FA
class clsFileUpload
{
...
class clsCaptcha {
    ...
    function clsCaptcha($Name, $Caption = "", $InitValue, & $Parent) {
        $this->Name       = $Name;
        $this->Parent     = & $Parent;
        $this->Visible    = true;
...
class clsLocale {
  ...
  function LoadTranslation($filename = "") {
    ...
    $this->Messages["text1"] = "Sample Date";
    $this->IsLoaded = true;
  }
